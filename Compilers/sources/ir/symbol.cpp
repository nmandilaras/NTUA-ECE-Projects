 ...
 * for Edsger Compiler - IR Generator
...
static SymbolEntry * newEntry (const char * name)
{
    ...
    for (e = currentScope->entries; e != NULL; e = e->nextInScope)
        if (strcmp(name, e->id) == 0) {
            error("Duplicate identifier (inside new Entry): %s", name);
            return NULL;
        }
...
SymbolEntry * newFunction (const char * name,llvm::Function* c)
{
    ...
    if (e == NULL) {
	    ...
	    e->u.eFunction.func = c;
            e->u.eFunction.firstArgument = NULL;
	    e->u.eFunction.lastArgument = NULL;
            e->u.eFunction.resultType = NULL;
	    e->u.eFunction.call_list = NULL;
	    e->u.eFunction.dec_head = NULL;
        }
        return e;
    }
...
void forwardFunction (SymbolEntry * f, struct dec_node *dec_head)
{
    ...
    f->u.eFunction.dec_head = dec_head;
}
...
llvm::Type *typeOf(Type type){
	switch (type->kind) {
		...
		case  TYPE_INTEGER:
			...
			return llvm::IntegerType::get(Mod->getContext(), 16);
		case  TYPE_REAL:
			//fprintf(stdout,"Type Double.\n");
			return llvm::Type::getX86_FP80Ty(Mod->getContext());
		case  TYPE_VOID:
