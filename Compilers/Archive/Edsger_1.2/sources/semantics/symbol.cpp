 ...
 *  
 * Parts appended by Nikiforos Mandilaras, Ioannis Siglidis
 * for Edsger Compiler - Semantic Analyzer
 *
 *
...
SymbolEntry * newVariable (const char * name, Type type)
{
	...
	e->u.eVariable.dynamic = false;
	e->u.eVariable.has_reference = false;
	e->u.eVariable.reference = NULL;
...
bool cmp_sym_root(SymbolEntry *s,SymbolEntry *p){
	SymbolEntry *r,*q;
	for(r = s; ((r->entryType== ENTRY_VARIABLE)?r->u.eVariable.reference!=NULL:r->u.eParameter.reference!=NULL); r=((r->entryType == ENTRY_VARIABLE)?r=r->u.eVariable.reference:r->u.eParameter.reference));
	for(q = p; ((q->entryType== ENTRY_VARIABLE)?q->u.eVariable.reference!=NULL:q->u.eParameter.reference!=NULL); q=((q->entryType == ENTRY_VARIABLE)?q->u.eVariable.reference:q->u.eParameter.reference));
	if((q == r)){
		if(q->entryType==ENTRY_VARIABLE){
			if(q->u.eVariable.type->kind == TYPE_ARRAY){
				return true;
			} else return false;
		} else return false;
	} else return false;
