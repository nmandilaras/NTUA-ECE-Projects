...
public:
   ...
   bool predict(ADDRINT ip, ADDRINT target)
   {
   ...
   // 000001 -> predict() == 0; 111110 -> predict() == 1
   void update(bool predicted, bool actual, ADDRINT ip, ADDRINT target)
   {
...
private:
   ...
   void gen_index_tag(ADDRINT ip, UINT32& index, UINT32 &tag)
   {
      index = (ip >> 4) & 0x3F;
      tag = (ip >> 10) & 0x3F;
   }
