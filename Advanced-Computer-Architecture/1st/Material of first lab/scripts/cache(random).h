...
class LRU 
{
 ...
 public:
    ...
    UINT32 Find(CACHE_TAG tag)
    {
             ...
             it != _tags.end(); ++it)
        {
            if (*it == tag) { // Tag found, lets make it MRU
                //_tags.erase(it);  //svhnei deikth, thesi toy array
                //_tags.push_back(tag); // enw edw vazei stoixeio se thesi sto telos
                return true;
            }
        }
	...
	int thee=0;
	    ...
	    thee = std::rand() % _associativity;
            ret = *(_tags.begin() + thee);
            _tags.erase(_tags.begin() + thee);
        }
        return ret;
    }
